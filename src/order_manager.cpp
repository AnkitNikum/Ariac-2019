...
{   InitializeEmptyBinCoord("pulley_part");
	...
	//std::string  empty_bin= camera_.GetEmptyBin();
        ...
        std::vector<std::string> empty_bin1=camera_.getEmptyBins();;
    std::string empty_bin;
    for(int i=0; i<empty_bin1.size(); i++){
               if(empty_bin1[i][4] =='3')
                     empty_bin = empty_bin1[i];
               else
                     empty_bin = empty_bin1[0];
     }
...
//conveyer_joint_pose_ = {-1.1800,0.0,-1.1, 1.9, 3.9, 4.7, 0};
	   ...
	   auto temp_pose=camera4_op_pose_;
	   temp_pose.position.z=temp_pose.position.z-0.1;
           ROS_INFO_STREAM("cam_pose"<<temp_pose); 
	   geometry_msgs::Pose temporaryPose;
	   temporaryPose.position.x = 1.193;
	   temporaryPose.position.y = 0.407;
	   temporaryPose.position.z = 1.175+0.1 ;
	   ...
	   while(1){   //for picking up from pulley
		    ...
		    up_pose.position.z=up_pose.position.z+0.1;
		    up_pose.position.y=up_pose.position.y+0.10;
                    ROS_INFO_STREAM("cam_pose"<<camera4_op_pose_); 
		    if (flag_==1){
			    ROS_INFO_STREAM("Moving down...");
			    ...
			    if (product_type=="pulley_part"){
		    		if ( camera4_op_pose_.position.y<0.425){
		    			ROS_INFO_STREAM("1st if");
		    			camera4_op_pose_.position.z=camera4_op_pose_.position.z+0.08-0.0110-0.006;
		    			/*camera4_op_pose_.position.y=camera4_op_pose_.position.y+0.00005-0.03; */  camera4_op_pose_.position.y=camera4_op_pose_.position.y-0.2;  			
		    		}
			    	if ( camera4_op_pose_.position.y>0.425){
			    		ROS_INFO_STREAM("2nd if");
		    			camera4_op_pose_.position.z=camera4_op_pose_.position.z+0.08-0.0140-0.006;
		    			/*camera4_op_pose_.position.y=camera4_op_pose_.position.y+0.00005-0.027;*/    
camera4_op_pose_.position.y=camera4_op_pose_.position.y-0.2;  	
		    		}
                             }
		    	...
		    	if(product_type=="gear_part"){
		    		if ( camera4_op_pose_.position.y<0.425){
		    			ROS_INFO_STREAM("1st if");
		    			camera4_op_pose_.position.z=camera4_op_pose_.position.z+0.0028;
		    			camera4_op_pose_.position.y=camera4_op_pose_.position.y-0.2;    			
		    		}
			    	if ( camera4_op_pose_.position.y>0.425){
			    		ROS_INFO_STREAM("2nd if");
...
camera4_op_pose_.position.y=camera4_op_pose_.position.y-0.2; 
			    ...
			    //ros::spinOnce();
                        if(camera4_op_pose_.position.x!=0)
                                       break;
		    //ros::Duration(0.25).sleep();
			n++;}
		}
...
void AriacOrderManager::CompletethisOrder(osrf_gear::Order order){
	...
	if(camera_.sensorBlackout()==false){
		auto repos_list=UpdateLists(shipment);
		
		for(int i=0;i<repos_list.size();i++){
		int r=-1;
		while(r==-1){
			ROS_INFO_STREAM("Reposition    "<<repos_list[i].first);
			r=RepositionComponents(repos_list[i],agv_id);
			if(r==-1)
  ...
  ROS_INFO_STREAM("Checking shipment for the last time");
  CheckOrder(shipment);
...
int AriacOrderManager::RepositionComponents(std::pair<std::string,std::vector<geometry_msgs::Pose> >diff_list,int agv_id){
    ...
    if(other_part_type_pose.first!="None"   ){// &&   other_part_type_pose.first!=diff_list.first   ){
		...
		while(1){
			...
			if(agv_id==1){
			if (repose_flag_==1){
				if (reposition_part_pose_back_.position.y < 2.83)
					reposition_part_pose_back_.position.y = 2.83;
				...
				ROS_INFO_STREAM("Reposition flag unsuccessfull");
				if (other_part_pose.position.y < 2.83) 
					other_part_pose.position.y = 2.83;
			...
			if(agv_id==2){
			if (repose_flag_dash_==1){
				if (reposition_part_pose_back_dash_.position.y < -2.845) 
					reposition_part_pose_back_dash_.position.y = -2.845;
				arm->GoToTarget({up_pose,reposition_part_pose_back_dash_});
		...
		while(1){
			...
			if (repose_flag_dash_==1){
				...
				ROS_INFO_STREAM("Reposition flag unsuccessfull");
				if (part_pose.position.y < -2.845) 
					part_pose.position.y = -2.845;
				arm->GoToTarget({up_pose,part_pose});
				part_pose.position.z-=0.005;
		...
		while(1){
			...
			if (repose_flag_==1){
				...
				ROS_INFO_STREAM("Reposition flag unsuccessfull");
				if (temp_pose.position.y < 2.83) 
					temp_pose.position.y = 2.83;
			...
			if (repose_flag_dash_==1){
				...
				ROS_INFO_STREAM("Reposition flag unsuccessfull");
				if (temp_pose.position.y < -2.845) 
					temp_pose.position.y = -2.845;
				arm->GoToTarget({up_pose,temp_pose});
				temp_pose.position.z-=0.005;
		...
		while(1){
	    	arm->GoToTarget(StampedPose_out.pose);
	    	ros::spinOnce();
	    	if((StampedPose_out.pose.position.y-arm->GetEEPose().position.y)<0.01 &&  (StampedPose_out.pose.position.x-arm->GetEEPose().position.x)<0.01   ){
	    		ROS_INFO_STREAM("State achieved");
	    		break;
	    	}
...
void AriacOrderManager::ExecuteOrder() {
    ...
    //Check empty bins
    std::vector<std::string> product_type1=camera_.getbinPartCount();;
...
if(empty_bin.size() != 0 && product_type1.size() == 1){
    ...
    while (1){
	    ...
	    ros::Duration(1.0).sleep();    //Waiting to see if there is a order update
	    ros::spinOnce();
	    if (order_update_flag_==1){
